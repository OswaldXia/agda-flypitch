...
record CoconeLanguage {D} (F : DirectedDiagramLanguage {u} {v} D) : Type (ℓ-max (ℓ-suc u) (ℓ-suc v)) where
  ...
  universalMap = record
    { funMorph = rec isSet𝔉 (λ (i , x) → funMorph (map i) x)
        λ (i , x) (j , y) → elim→Set (λ _ → isProp→isSet (isSet𝔉 _ _))
          (λ (k , z , i~k , j~k , H₁ , H₂) → eqToPath $ begin
            ...
            funMorph (map j) y                        ∎)
          (λ _ _ → isSet𝔉 _ _ _ _)
    ; relMorph = rec isSetℜ (λ (i , x) → relMorph (map i) x)
        λ (i , x) (j , y) → elim→Set (λ _ → isProp→isSet (isSetℜ _ _))
          (λ (k , z , i~k , j~k , H₁ , H₂) → eqToPath $ begin
