...
_⨭_ : {A : Set a} (P : Pred A ℓ) (x : A) → Pred A (ℓ ⊔ a)
P ⨭ x = P ∪ ｛ x ｝
...
_⟦_⟧ : {A : Set a} {B : Set b} (f : A → B) (P : Pred A ℓ) → Pred B (ℓ ⊔ a ⊔ b)
f ⟦ P ⟧ = λ y → ∃[ x ] x ∈ P × y ≡ f x
...
_⟦｛_｝⟧ : {A : Set a} {B : Set b} (f : A → B) (x : A) → Pred B (a ⊔ b)
f ⟦｛ x ｝⟧ = f ⟦ ｛ x ｝ ⟧
...
⋃_ : {A : Set a} {B : Set b} → (A → Pred B ℓ) → Pred B (a ⊔ ℓ)
(⋃ Pᵢ) x = ∃[ i ] Pᵢ i x

replacement-syntax : (A : Set a) {B : Set b} → (A → B) → Pred B (a ⊔ b)
